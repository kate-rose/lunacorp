...
**2026-07-29 — v0.5, tutorial UX pass (browser-verified, desktop + mobile).** Built against standard game/software onboarding practice: *show-don't-tell / learn-by-doing, one coach mark at a time on primary actions, progressive disclosure, visual cues (arrow + pulse), player-controlled pacing.*
- **Filmstrip is now navigable both ways** — ◀ BACK / NEXT ▶ buttons, clickable numbered dots (jump to any card), `←`/`→`/Backspace/PageUp-Down keys, "CARD n / 8" counter. Frame is a flex column so the nav bar never scrolls out of reach. Click-anywhere-to-advance was removed (it fought the new controls).
- **New MODULE 3 — ANATOMY OF A MESSAGE**: an annotated fake message card with dashed **highlight boxes + numbered pins (1–7)** over the display name, address (solid cyan "hot" box — the one you click), subject, link, attachment, status bar, and the three stamps, plus a numbered legend. Teaches the interface *before* the player is asked to use it under time pressure.
- **Colour words render in their colour** via a small author-only markup in tutorial/coach copy: `{g:…}` `{r:…}` `{b:…}` and `**bold**` (`rich()` in app.ts — deliberately unescaped, never fed player input).
- **Coach marks are now anchored callouts, not a fixed corner box.** `positionCoach()` measures the target after each render and places the callout right/left/below it with a CSS arrow pointing at it; the target gets a pulsing cyan outline (`.coach-target`). Steps are `{text, target}` objects (`CoachStep`), so each beat points at the exact element it's describing. Callouts have **STEP n / m + BACK/NEXT**, and re-anchor on window resize.
- **The practice message is now coached** (4 anchored steps: FROM line → *click the address* → read the body → the red QUARANTINE stamp), which is what the "Click the sender's address" pointer request was really about. Messages 1–3 keep their coaching, now anchored too.

**2026-07-29 — v0.6, after first team playtest.** Feedback: ferret and tutorial landed, ELEANOR is a favourite.
- **The web-address lesson got its own card** (orientation card 5), since "which part of the URL is the real site" is the hardest idea in the deck. Labelled breakdown (decoration {red} / THE REAL SITE {cyan} / just the page {dim}), the rule in one plain line ("Find the first single slash. The real site is the name just before it"), then a **click-the-owner drill** over three links: a plain real one, the subdomain trap, and a hyphen lookalike. Wrong answers name the trap explicitly. Drill data is `URL_DRILLS` in orientation.ts.
- **FERRET.EXE is a live desktop pet** (`src/ferret.ts`): wanders, naps, speaks, pettable, and reacts to your stamps via `ferretReact()`. Text sprite in one constant, so swapping in pixel art means replacing `SPRITE` + the `.ferret-sprite` CSS and keeping the state names (walking/asleep/petted/flip). Lives outside `#app` so re-renders never destroy it.
- **ELEANOR has two states** (`src/data/eleanor.ts`): thirsty (sparse drooping leaves, dim green) and, once watered, a full spider plant with arching fronds, drooping tips, and a baby spiderette on a runner (bright green + perk animation). Watering now has a visible payoff.
- **Copy trim started**: orientation cut 8 cards → 7 while *adding* the URL lesson, rewritten plain ("This game teaches you to spot phishing" over "FILMSTRIP No. 7"), em-dashes thinned. **Still to do: the same pass over the 13 messages' field notes and outcome bodies**, which is where most remaining sci-fi ornamentation lives.
